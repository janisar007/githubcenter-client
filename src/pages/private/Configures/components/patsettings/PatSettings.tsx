    ...
    <div>
      <div className="w-[80%] bg-vol-50 rounded-lg">
        <div className="px-5 py-4 flex flex-col gap-1">
          <span className="font-medium">Your PAT(Personal Access Token)</span>
          ...
          <div className="flex justify-between items-center px-5 py-5">
            <div className="text-cgray-dtext flex flex-col gap-2">
              <div className="font-medium flex items-center gap-2">
                ...
                <span onClick={() => setIsRenamePatOpen(true)} className="hover:text-blue-600 cursor-pointer"><GoPencil/></span>


              </div>
              <div className="text-xs flex  gap-[0.10rem] text-cgray-ntext">
                <span>{`Created at ${patData?.createdAt && formatDate(patData?.createdAt)}`}</span>
                <BsDot />
                <span>{`Updated on ${patData?.updatedAt && formatDate(patData?.updatedAt)}`}</span>
              </div>
            </div>
